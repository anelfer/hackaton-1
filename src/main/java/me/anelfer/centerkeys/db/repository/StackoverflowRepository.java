...
public interface StackoverflowRepository extends JpaRepository<StackoverflowEntity, Long> {
    ...
    @Query(value = "select tag, max(today) as today, max(id) as id, max(week) as week, max(total) as total, " +
            ...
            " and now() - timestamp < interval '2 days' order by total desc) as abc group by tag order by total desc limit 100;",
            nativeQuery = true)
    ...
    @Query(value = "select tag, max(today) as today, max(id) as id, max(week) as week, max(total) as total, " +
            "max(timestamp) as timestamp " +
            "from (select * from stackoverflow where extract(day from timestamp) = extract(day from now())" +
            " and now() - timestamp < interval '2 days' order by week desc) as abc group by tag order by week desc limit 100;",
            nativeQuery = true)
    ...
    @Query(value = "select tag, max(today) as today, max(id) as id, max(week) as week, max(total) as total, " +
            "max(timestamp) as timestamp " +
            "from (select * from stackoverflow where extract(day from timestamp) = extract(day from now())" +
            " and now() - timestamp < interval '2 days' order by today desc) as abc group by tag order by today desc limit 100;",
            nativeQuery = true)
